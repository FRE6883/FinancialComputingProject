...
namespace fre{
	...
	void FetchStockData_test(map<string, Stocks> &stockData)
	{
		...
		if (handle)
		{
			...
			cout<<"start FetchStockData_test"<<endl;
			for(map<string, Stocks>::iterator it = stockData.begin(); it!=stockData.end(); it++) 
			{	
				...
				string start_date = it->second.GetStartDate();
				string end_date = it->second.GetEndDate();
				struct MemoryStruct data;
				data.memory = NULL;
				data.size = 0;
